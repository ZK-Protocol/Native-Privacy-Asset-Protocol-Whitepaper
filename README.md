...
```solidity
interface IZRC20 {
    /// @notice Mint privacy tokens
    /// @param proof zk-SNARK proof generated by circuits
    /// @param publicInputs Public inputs corresponding to the proof
    function mint(bytes calldata proof, bytes calldata publicInputs) external;

    /// @notice Privacy transfer
    /// @param proof zk-SNARK proof generated by circuits
    /// @param publicInputs Public inputs corresponding to the proof
    /// @param encryptedNotes Encrypted information of output notes (for recipient decryption)
    function transfer(
    ...
    /// @notice New Commitment added to Merkle Tree
    event NewCommitment(bytes32 indexed commitment, uint32 leafIndex, uint256 timestamp);

    /// @notice Nullifier registered, marking note as spent
    event NewNullifier(bytes32 indexed nullifier, uint256 timestamp);

    /// @notice Transfer occurred, only publicHash exposed on-chain
    event TransferOccurred(bytes32 publicHash);
}
```
...
- Triggers `NewCommitment` event upon success
...
- Upon successful execution:
  - Triggers `NewNullifier` event (marking old notes as void)
  - Triggers `NewCommitment` event (generating new commitments)
  - Triggers `TransferOccurred` event (only exposing a public hash for on-chain verification without revealing amounts and addresses)

**Event Mechanism**
- **NewCommitment**: Clients sync new privacy notes by monitoring this event
- **NewNullifier**: Clients record whether notes are spent to prevent double-spending
- **TransferOccurred**: Only exposes an irreversible hash value externally, protecting transfer details
...
- **60% reduction in Gas costs** (compared to traditional privacy solutions)
- **40% reduction in client resources**
- **50% reduction in network bandwidth**
- **Parallel Finalized Transfers** increase throughput to hundreds of TPS
